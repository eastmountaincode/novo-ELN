...
const candidates = report.findings
  .filter((finding) => finding.classification === "manual_review")
...
async function repairFinding(finding, enml) {
  ...
  const currentPlainText = currentEditorPlainText(page.body);
  if (!currentPlainText.ok) return skipped(finding, currentPlainText.reason);
  const preservation = currentTextIsPreserved(currentPlainText.text, plainText);
  if (!preservation.ok) {
    return skipped(finding, `current body has text not present in fixed ENEX conversion: ${preservation.missing.slice(0, 3).join("; ")} || FIXED_FIRST_LINES: ${textLines(plainText).slice(0, 8).join(" | ")} || CURRENT_FIRST_LINES: ${textLines(currentPlainText.text).slice(0, 8).join(" | ")}`);
  }
...
function currentEditorPlainText(body) {
  try {
    return { ok: true, text: editorDocumentToPlainText(JSON.parse(body || "{}")).trim() };
  } catch (error) {
    return { ok: false, reason: `current page body is not valid editor JSON: ${error.message}` };
  }
}

function currentTextIsPreserved(currentText, fixedText) {
  const currentLines = textLines(currentText);
  const fixedLines = textLines(fixedText);
  let start = 0;
  const missing = [];
  for (const line of currentLines) {
    let found = -1;
    for (let i = start; i < fixedLines.length; i += 1) {
      if (fixedLines[i] === line) { found = i; break; }
    }
